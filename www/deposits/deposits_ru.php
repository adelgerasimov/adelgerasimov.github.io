...
?>
	<div class="ref_page"><form method="post" name="refsys_frm"><fieldset><table class="formatTable">				<tbody><tr>							<td>Всего депозитов на сумму</td><td><span class="value">$<?php echo $s; ?></span></td></tr>				<tr>							
<td>Следующее зачисление % в</td><td><span class="value"><a href="#" target="_blank"><span id="deptimer"><?php echo $row['id']; ?></span></b> <?php echo $sevenairlines; ?></a></span></td></tr>				<tr>				<td colspan="5"><table class="FormatTable" border="1"><tbody><tr><th>Сумма</th><th>Процент</th><th>Период зачисления</th><th>Срок</th><th>Дата открытия</th></tr><tr></tr>
...
while($row = mysql_fetch_array($result)) {
	...
	$row2		= mysql_fetch_array($result2);
?>
<tr><td colspan="0" align="center">$<?php echo $row['sum']; ?></td><td colspan="0" align="center"><?php echo $row2['percent']; ?>%</td><td colspan="0" align="center">В день</td><td colspan="0" align="center">30 дней</td><td colspan="0" align="center"><?php echo date("d.m.Y H:i", $row['date']); ?></td></tr>
<?php } ?>
