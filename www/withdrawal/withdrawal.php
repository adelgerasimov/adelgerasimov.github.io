...
if ($login) {
	...
	if($_GET['cancel']) {
			...
			if($r2['sum']) {
				...
				print '<center><p class="erok">The application is canceled, money will be returned to the balance</p></center>';
			} else {
				print '<center><p class="er">You can not cancel a request</p></center>';
			}
	}
	...
	if ($_GET['action'] == 'save') {
		...
		if ($sum <= 0) {
			print '<center><p class="er">Please correct sum (from $'.$cfgMinOut.' to $'.cfgSET('cfgMaxOut').')!</p></center>';
		} elseif ($sum < $cfgMinOut || $sum > cfgSET('cfgMaxOut')) {
			print '<center><p class="er">At a time is allowed to withdraw from $'.$cfgMinOut.' to $'.cfgSET('cfgMaxOut').'!</p></center>';
		} elseif ($r['pm_balance'] < $sum) {
			print '<center><p class="er">You do not have enough money in the account!</p></center>';
...
//			print '<center><p class="er">Today you have reached your limit requests for withdrawal. Please try tomorrow.</p></center>';	
		} elseif($ps < 1) {
			print '<center><p class="er">Specify the payment system! Enter the account number in your profile.</p></center>';
		} elseif(!$purse) {
			print '<center><p class="er">Enter account number.</p>';
			...
			if (mysql_query($sql)) {
					...
					// АВТОВЫПЛАТЫ
						if($ps == 1 && $cfgAutoPay == "on") {
							...
							if($f===false){
								...
								print '<center><p class="er">Not available payment gateway PerfectMoney. Please try again later.</p></center>';
							} else {
								// getting data
								$out=array(); $out="";
								...
								if(!preg_match_all("/<input name='(.*)' type='hidden' value='(.*)'>/", $out, $result, PREG_SET_ORDER)){
									...
									print '<center><p class="er">The payment is denied by Perfect Money</p></center>';
							...
							if ($payeer->isAuth()) {
								...
								if(!empty($arTransfer["historyId"])) {
									print "<center><p class=\"erok\">Withdraw №".$arTransfer["historyId"]." successfully.</p></center>";
								} else {
									mysql_query('UPDATE `output` SET status = 0 WHERE id = '.$lid.' LIMIT 1');
									print '<center><p class=\"er\">ERROR! Application will be made in the manual mode.</p></center>';		
								}
							} else {
								mysql_query('UPDATE `output` SET status = 0 WHERE id = '.$lid.' LIMIT 1');
								print "<center><p class=\"er\">ERROR! Application will be made in the manual mode.</p></center>";
					...
					print '<center><p class="erok">Your application is sent to the processing!</p></center>';

			} else {
				print '<center><p class="er">Unable to send a request to withdraw the money!</p></center>';
			}
		}
	}
	?>
...
<br>
<font size="4" color="red">Something went wrong...</font>
<br>
<?php } ?>
...
<font>Your balance <?php echo $pmbalance; ?> <b>$</b></font>
	<form method="post" action="?action=save" name="add"><fieldset><table class="formatTable">									<input name="Oper" id="add_Oper" value="CASHOUT" type="hidden">				<tbody><tr>							<td><label for="add_PSys"><span class="descr_req">Pay system<span class="descr_star">*</span></span></label></td><td><select name="ps" id="add_PSys" onChange="CheBal();" class="select">
<?php
if($r['pm']) {
...
</select><font size='2'>Tip: Enter your details to set up a profile for a wider choice of payment systems for output.</font></td></tr>				<tr>							<td><label for="add_Sum"><span class="descr_req">Sum $<span class="descr_star">*</span></span></label></td><td><input name="sum" id="add_Sum" value="" size="10" type="text" class="float"> <i><span id="ccurr"></span></i></td></tr></tbody></table></fieldset><input name="__Cert" value="0eea8f1e" type="hidden"><br><input name="add_btn" value="Создать заявку" type="submit" class="button-blue"></form>
</tbody></table>
...
else
{
	?>
	<center>
	<span style="font-family: 'arial black', 'avant garde'; font-size: medium;"><span style="color: #333333;">Cash withdrawal is possible only after the discovery of the Deposit.</span><p></p>
	</center>
	<br><br>
	<?
}
...
<?php
                ...
                if ( mysql_num_rows($result) == 0 ){ echo "<td colspan='0' align='center'>The history of your conclusions is empty!</td>"; }
		while ($row = mysql_fetch_array($result)) {
?>
<tr>
...
<td colspan="0" align="center">
		<?php if($row['status'] == 0) {
			print '<span class="tool"><span class="tip"><b>The application is processed.</b></span></span>';
		} elseif($row['status'] == 2) {
			print '<span class="tool"><span class="tip"><b>Application is made!</b></span></span>';
		} elseif($row['status'] == 6) {
			print '<span class="tool"><span class="tip"><b>Application canceled.</b></span></span>';
		} else {
